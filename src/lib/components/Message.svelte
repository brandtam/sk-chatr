...
<div class="text-gray-100 chat {type === 'user' ? 'chat-end' : 'chat-start'} justify-end">
	<div class="flex items-start p-2 text-sm">
		<img src="{type === 'assistant' ? urlFor($selectedMember.image).width(40).height(40).fit('crop').auto('format').url() : 'https://github.com/identicons/chatbot.png' }" class="w-10 h-10 rounded mr-3" alt="{type} Profile Picture">
