...
<div class="bg-purple-800 text-purple-lighter flex-none w-64 pb-6 hidden md:block">
	...
	<div class="mb-8">
		...
		{#each bots as bot}
			<div on:click|preventDefault={() => handleSelect(bot.name)} class="flex items-center mb-3 px-4 cursor-pointer  {bot.name === selected ? 'bg-purple-700' : 'hover:bg-purple-900'}">
				<span class="bg-green rounded-full block w-2 h-2 mr-2 {bot.name === selected ? 'border border-white' : ''} "></span>
